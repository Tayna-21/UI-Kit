		...
		<div className={styles.colorPickerWrapper}>
            ...
            <div className={styles.colorPickerContainer}>
				{colors && colors.map((color, index) => 
				    <input key={index} type='radio' value={color} style={{backgroundColor: `${color}`}} checked={ color === selectedColor ? true : false } onClick={selectColor}/>
				)}
				<div className={styles.paletteWindow} style={{background: `${createdColor}`}} role='button' onClick={showPalette}></div>
                    {paletteVisible && 
				    <div className={styles.paletteWrapper}>
				        <SketchPicker color={createdColor} onChangeComplete={createNewColor} />
					</div>}
			</div>
		</div>
	)
